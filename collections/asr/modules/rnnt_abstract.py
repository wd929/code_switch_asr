...
class AbstractRNNTJoint(NeuralModule, ABC):
    ...
    @abstractmethod
    def joint_after_projection(self, f: torch.Tensor, g: torch.Tensor, prob_tensor: torch.Tensor) -> Any:
        """
    ...
    def joint(self, f: torch.Tensor, g: torch.Tensor, prob_tensor: torch.Tensor) -> torch.Tensor:
        """
        ...
        Returns:
            Logits / log softmaxed tensor of shape (B, T, U, V + 1).
        """
        return self.joint_after_projection(self.project_encoder(f), self.project_prednet(g), prob_tensor)
